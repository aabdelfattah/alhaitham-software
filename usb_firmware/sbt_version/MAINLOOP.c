...
#define MAX_CLASS_REQUEST    0x0C
code void (*ClassDeviceRequest[])(void) =
{
	Chap9_StallEP0,//0x00
	Chap9_GetReport,//0x01
	Chap9_GetIdle,//0x02
	Chap9_GetProtocol,//0x03
	Chap9_StallEP0,//0x04
	Chap9_StallEP0,//0x05
	Chap9_StallEP0,//0x06
	Chap9_StallEP0,//0x07
	Chap9_StallEP0,//0x08
	Chap9_SetReport,//0x09
	Chap9_SetIdle,//0x0A
	Chap9_SetProtocol//0x0B  
};
...
//   == sizeof(DEVICE_REQUEST) )
  {
    ...
    ////printf("j = size of device request");
...
void DeviceRequest_Handler(void)
{
 ...
 //   //printf("type = 0x%02x, req = 0x%02x\n", type, req);
    ...
    help_devreq(type, req ,(USHORT)ControlData.DeviceRequest.wValue ); /* print out device request */
...
void help_devreq(UCHAR type, UCHAR req, USHORT wValue)
{
  UCHAR typ = type;
  typ >>= 5;
 ...
 // printf("Request Type = %s, Request = %s Requestno = %x.\n", _NAME_USB_REQUEST_TYPE[typ],
   //  _NAME_USB_STANDARD_REQUEST[req],req);
  }
  else if(type == USB_CLASS_REQUEST)
  {

  //printf("Request Type = %s, Request = %s Requestno = %x.\n", _NAME_USB_REQUEST_TYPE[typ],
    // _NAME_USB_CLASS_REQUEST[req],req);
   //printf("wValue = %x. \n",wValue );
  }
  else
  {
